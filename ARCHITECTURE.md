...
4. Stages `translations.json`, every `data/*.json` file (including `data/scraper-runs.json`, never the SQLite DB), a generated `_headers`, and the advanced-mode Pages `_worker.js` into a temporary bundle under the OS temp directory. `metadata.json` includes a deterministic observation `snapshotId`.
...
The generated `_worker.js` serves the staged files through the Pages `ASSETS` binding and applies CORS only to `translations.json` and `data/*.json`. It allows exactly `https://dh-sandbox-web.quintype.io`, `https://www.prajavani.net`, `https://prajavani-web.qtstage.io`, `https://www.deccanherald.com`, and `https://product-tpml.github.io`; browser requests from other origins receive `403`, while requests without an `Origin` header remain available to the scraper and Apps Script importer.
...
- Runtime translations and `data/*.json` files are served from the Cloudflare Pages data project at `https://agro-dashboard-data.pages.dev`; the dashboard host and the data project are independent. The data project applies an exact-origin CORS allowlist through its generated advanced-mode Worker.
...
- Both static HTML entry points (`index.html` and `cms-migration/index.html`) load Google Tag Manager container `GTM-MGBFQ9R`; the noscript fallback is placed immediately inside each body. The browser pushes analytics events to `window.dataLayer` through `analytics.js`, while the registered `card_details` custom dimension remains deployment/measurement configuration. Page-view attribution is retained in browser history state so back/forward navigation does not add analytics fields to URLs.
